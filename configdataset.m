...
%     params.ext = '.jpg';
%     params.qext = '.jpg';
%     params.dir_data= [dir_main 'oxford105k/'];
%     cfg = config_oxford (params);
...
%     params.ext = '.jpg';
%     params.qext = '.jpg';
%     params.dir_data= [dir_main 'paris106k/'];
%     cfg = config_paris (params);
...
cfg.dir_images = sprintf ('%s/', cfg.dir_data);
...
%----------------------------------------------------
  ...
  if isempty(ext)
    fname = sprintf ('%s%s%s', cfg.dir_data, cfg.imlist{i}, cfg.ext);
  else
    fname = sprintf ('%s%s', cfg.dir_data, cfg.imlist{i});
  end
...
%----------------------------------------------------
  ...
  if isempty(ext)
    fname = sprintf ('%s%s%s', cfg.dir_data, cfg.qimlist{i}, cfg.qext);
  else
    fname = sprintf ('%s%s', cfg.dir_data, cfg.qimlist{i});
  end
